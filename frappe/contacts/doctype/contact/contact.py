...
class Contact(Document):
    ...
    def validate(self):
        ...
        if self.get("sync_with_google_contacts") and not self.get("google_contacts"):
            frappe.throw(
                _("Select Google Contacts to which contact should be synced."))
    ...
    def has_common_link(self, doc):
        reference_links = [(link.link_doctype, link.link_name)
    ...
    def add_email(self, email_id, is_primary=0, autosave=False):
        if not frappe.db.exists("Contact Email", {"email_id": email_id, "parent": self.name}):
            self.append(
                "email_ids", {"email_id": email_id, "is_primary": is_primary})
    ...
    def set_primary(self, fieldname):
        ...
        is_primary = [
            phone.phone for phone in self.phone_nos if phone.get(field_name)]
...
def download_vcards(contacts: str):
    ...
    make_access_log(
        doctype="Contact",
        filters=json.dumps([["name", "in", contact_ids]],
...
def get_contact_name(email_id):
    contact = frappe.get_all("Contact Email", filters={
                             "email_id": email_id}, fields=["parent"], limit=1)
...
def get_contacts_linked_from(doctype, docname, fields=None):
    ...
    contact_names = frappe.get_value(doctype, docname, fieldname=[
                                     f.fieldname for f in link_fields])
...
def get_full_name(
    ...
    full_name = " ".join(
        filter(None, [cstr(f).strip() for f in [first, middle, last]]))
    ...
    for contact in contact_list:
        contact["email_ids"] = frappe.get_all(
            ...
            filters={"parenttype": "Contact",
                     "parent": contact.name, "is_primary": 0},
            fields=["email_id"],
        )
