...
def get_value(doctype, fieldname, filters=None, as_dict=True, debug=False, parent=None):
    ...
    if not frappe.has_permission(doctype, parent_doctype=parent):
        frappe.throw(_("No permission for {0}").format(
            _(doctype)), frappe.PermissionError)
    ...
    if frappe.get_meta(doctype).issingle:
        value = frappe.db.get_values_from_single(
...
def set_value(doctype, name, fieldname, value=None):
    ...
    if not frappe.get_meta(doctype).istable:
        ...
        doc = frappe.db.get_value(
            doctype, name, ["parenttype", "parent"], as_dict=True)
...
def bulk_update(docs):
    ...
    for doc in docs:
        ...
        try:
            ...
            failed_docs.append(
                {"doc": doc, "exc": frappe.utils.get_traceback()})
    ...
    try:
        ...
        frappe.msgprint(
            _("You need {0} permission to fetch values from {1} {2}").format(
                frappe.bold(_("Read")), frappe.bold(
                    doctype), frappe.bold(docname)
            ),
...
def insert_doc(doc) -> "Document":
    ...
    if frappe.is_table(doc.doctype):
        if not (doc.parenttype and doc.parent and doc.parentfield):
            frappe.throw(
                _("Parenttype, Parent and Parentfield are required to insert a child record"))
